...
page({
    onShow() {
        const children = this.selectComponent('.child')
        children.setData({})  // 修改子组件的状态
        children.addCount()   // 调用子组件的方法
    }
})
```
...
wxs类似与JavaScript，与JavaScript有写不同

- 结合 `wxml` 使用，类似与 `html` 中的 `script` 标签
...
<wxs module="m3" src="/utils/random.wxs"></wxs>
```
...
- 使用
  ...
  Page({
      async onLoad() {
          // console.log(this)
          const res = await wx.p.showLoading({
              title: '加载中...',
          })
          console.log(res)
      }
...
- `mobx-miniprogram`：创建包管理器，store实例
- `mobx-miniprogram-bindings`：共享数据

安装

- ```shell
  npm i mobx-miniprogram@4.13.2 mobx-miniprogram-bindings@1.2.1 -S
  ```
...
- 引用
  ...
  - 在页面中的使用
    ...
    Page({
         onLoad() {
            this.a = createStoreBindings(this, {
                store,
                // fields: ['num1', 'num2', 'sum'],   // 数组写法
                fields: {  // 对象写法
                    num1: 'num1',
                    num2: () => store.num2,
                    sum: store => store.sum
            ...
            console.log(this.updateNum1(2))
        },
        onUnload() {
            // 卸载页面时解除绑定 
            this.a.destroyStoreBindings()
        }
    })
    // 此时，可以直接通过this.num1, this.sum, this.updateNum1()来使用了
    ```
...
- 目录结构
  ...
  `packageA/pages/packageA-test/index`
...
- 小程序中不支持使用 `axios` 发送请求

- 原生的 `wx.request()` 功能单一，不支持拦截器

- 在 `uniapp` 项目中只用第三方包 `@escook/request-miniprogram`

  `npm` 地址：https://www.npmjs.com/package/@escook/request-miniprogram
